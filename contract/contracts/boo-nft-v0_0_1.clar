...
(define-non-fungible-token BTC-NFT-Coll uint)
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? BTC-NFT-Coll token-id))
)
...
(define-public (transfer
  ...
  (begin
    ...
    (nft-transfer? BTC-NFT-Coll token-id sender recipient)
  )
)
...
(define-public (mint (recipient principal))
  ...
  (let ((token-id (+ (var-get last-token-id) u1)))
    ...
    (try! (nft-mint? BTC-NFT-Coll token-id recipient))
